...
;(function($, window, undefined)
{
	...
	$(document).ready(function()
	{
		...
		neonLogin.$container.validate({
			...
			highlight: function(element){  //元素出错时调用
				$(element).closest('.input-group').addClass('validate-has-error');
			},
			
			
			unhighlight: function(element)//元素正常调用
			{
				$(element).closest('.input-group').removeClass('validate-has-error');
			},
			
			submitHandler: function(ev)  //提交表单调用
			{
				
				$(".login-page").addClass('logging-in'); // 隐藏登录表单，并显示进度条
				...
				setTimeout(function()
				{
					...
					neonLogin.setPercentage(40 + random_pct);//设置进度条
					...
					$.ajax({
						...
						data: {
							...
							rememberMe: $("input#flexSwitchCheckChecked").prop("checked"),
						},
						error: function(errr)
						{
							console.log(errr);
