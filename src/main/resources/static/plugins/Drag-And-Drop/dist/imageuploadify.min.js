...
(function($, window, document, undefined) {
	...
	$.fn.imageuploadify = function(opts) {
		...
		this.each(function() {
			...
			let totalFiles = [];
			let counter = 0;
			let dragbox = $(`<div class="imageuploadify well"><div class="imageuploadify-overlay"><i class="fa fa-picture-o"></i></div><div class="imageuploadify-images-list text-center"><i class="bx bxs-cloud-upload"></i><span class='imageuploadify-message'>请点击右上方按钮上传所选文件(请勿上传同名文件)</span><button type="button"class="btn btn-default test1">选择要上传的文件</button></div></div>`);
			...
			const readingFile = (id, file) => {
				...
				let container = $(`<div  class='imageuploadify-container'><button mark='`+mark+`' type='button'class='btn btn-danger bx bx-x'></button><div class='imageuploadify-details'><span>${file.name}</span><span>${file.type}</span><span>${file.size}</span></div></div>`);
				...
				if (file.type && file.type.search(/image/) != -1) {
				...
				} else if (file.type && file.type.search(/video/) != -1) {
					fReader.onloadend = function(e) {
						let image = $("<img>");
						image.attr("src","static/images/videoicon.jpeg");
						container.append(image);
						imagesList.append(container);
						imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+4)").css("margin-left", marginSize + "px");
						imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+3)").css("margin-right", marginSize + "px");
					};
				}else if (file.type && file.type.search(/audio/) != -1) {
					fReader.onloadend = function(e) {
						let image = $("<img>");
						image.attr("src","static/images/audioicon.jpg");
						container.append(image);
						imagesList.append(container);
						imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+4)").css("margin-left", marginSize + "px");
						imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+3)").css("margin-right", marginSize + "px");
					...
					fReader.onloadend = function(e) {
						let image = $("<img>");
						image.attr("src","static/images/fileicon.jpg");
						container.append(image);
						imagesList.append(container);
						imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+4)").css("margin-left", marginSize + "px");
						imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+3)").css("margin-right", marginSize + "px");
				...
				xhr.onreadystatechange = function(e) {
					if (xhr.status == 200 && xhr.readyState === XMLHttpRequest.DONE) {
						...
						if(respone.status == 'success'){
							...
							// $("#page-wrapper").load("uploadfile #page-wrapper");
							window.location.replace("uploadfile");
						}else if(respone.status == 'invalid'){
							alert(respone.message)
						}
